...
ParticleContainer ParticleGenerator::createCuboid(std::array<double, 3> x, std::array<double, 3> v,
                                                  std::array<int, 3> N, double h, double m, double sig, double eps, int type, bool fixed) {
    ...
    for (int z_i = 0; z_i < N[2]; z_i++) {
        for (int y_i = 0; y_i < N[1]; y_i++) {
            for (int x_i = 0; x_i < N[0]; x_i++) {
                cuboid.addParticle(coordinate, v, m, type, sig, eps, fixed);
                coordinate[0] += h;
            }
            coordinate[0] = x[0];
            coordinate[1] += h;
...
void ParticleGenerator::createCuboidInCells(std::array<double, 3> x, std::array<double, 3> v,
    ...
    for (int z_i = 0; z_i < N[2]; z_i++) {
        for (int y_i = 0; y_i < N[1]; y_i++) {
            for (int x_i = 0; x_i < N[0]; x_i++) {
                cells.addParticle(x_axis_tmp, y_axis_tmp, z_axis_tmp, coordinate, v, m, type, sig, eps, fixed);
 ...
 */
ParticleContainer ParticleGenerator::createDisk(std::array<double, 3> center, std::array<double, 3> v, double m, int r, double h, double sig, double eps, int type, bool fixed) {
    ...
    //Add center
    disk->addParticle({center[0], center[1] , center[2]},v,m,type, sig, eps, fixed);
    ...
    for (double x_minus = center[0] - h, x_plus = center[0] + h; x_plus < center[0] + radius - (h/4); x_minus -= h, x_plus += h) {
        disk->addParticle({x_minus, center[1], center[2]},v,m,type, sig, eps, fixed);
        disk->addParticle({x_plus, center[1], center[2]},v,m,type, sig, eps, fixed);
    }

    //Add particles for top, bottom, left and right
    disk->addParticle({center[0] + x, center[1] + y, center[2]},v,m,type, sig, eps, fixed);
    disk->addParticle({center[0] + x, center[1] - y, center[2]},v,m,type, sig, eps, fixed);
    disk->addParticle({center[0] + y, center[1] + x, center[2]},v,m,type, sig, eps, fixed);
    disk->addParticle({center[0] - y, center[1] - x, center[2]},v,m,type, sig, eps, fixed);
    ...
    while (x < y + (h/4.0)) {
        ...
        if (!end) {
            ...
            for (double x_temp = center[0] - x + h; fill && x_temp < center[0] + x - (h/4.0); x_temp += h) {
                disk->addParticle({x_temp, center[1] + y, center[2]},v,m,type, sig, eps, fixed);
            }
            disk->addParticle({center[0] + x, center[1] + y, center[2]},v,m,type, sig, eps, fixed);
        ...
        for (double x_temp = center[0] - y + h; x_temp < center[0] + y - (h/4.0); x_temp += h) {
            disk->addParticle({x_temp, center[1] + x, center[2]},v,m,type, sig, eps, fixed);
        }
        disk->addParticle({center[0] + y, center[1] + x, center[2]},v,m,type, sig, eps, fixed);
 ...
 * @return particle container containing the newly created particles
 */
ParticleContainer ParticleGenerator::createDiskAlternative(std::array<double, 3> center, std::array<double, 3> v, double m, int r, double h, double sig, double eps, int type, bool fixed) {
    ParticleContainer disk = ParticleContainer();
    double radius = r * h;
    //Create cuboid around center
    ParticleContainer cuboid = createCuboid({center[0] - radius, center[1] - radius, center[2]}, v, {2 * r + 1, 2 * r + 1, 1},h,m, sig, eps, type, fixed);
...
void ParticleGenerator::createDiskInCells(std::array<double, 3> center, std::array<double, 3> v, double m,
    ...
    ParticleContainer container = ParticleGenerator::createDiskAlternative(center,v,m,r,h, sig, eps, type, fixed);
...
ParticleContainer ParticleGenerator::createSphere(std::array<double, 3> center, std::array<double, 3> v, double m,
                                                  int r, double h, double sig, double eps, int type, bool fixed) {

    ParticleContainer sphere = ParticleContainer();
    double radius = r * h;
    //Create cuboid around center
    ParticleContainer cuboid = createCuboid({center[0] - radius, center[1] - radius, center[2] - radius}, v, {2 * r + 1, 2 * r + 1, 2 * r + 1},h,m, sig, eps, type, fixed);
    //Only add particles that are in radius of center
    for (const Particle &p : cuboid) {
        if (ArrayUtils::L2Norm(p.getX() - center) <= radius + (h/4.0)) {
