...
void PositionCalculator::PositionStoermerVerletCell(LinkedCellContainer &grid, double delta_t, double cutoff, int index) {
    for (int i = 0; i < grid.cell_numbers(); i++) {
        if(index == 1275&&i==1512) {
            spdlog::info("lookes in cell: {}", i);
        }
        for(int j = 0; j < grid.Particles_in_cell(i); j++) {
           ...
           int new_cell = i;
           int x_old = floor(i% grid.getXMax());
           int y_old = floor(i / grid.getXMax());
           int z_old = floor(i /(grid.getXMax()*grid.getYMax()));
           int x_now =  floor(x_new[0] / cutoff);
           int y_now = floor(x_new[1] /cutoff);
           int z_now = floor(x_new[2] /cutoff);
           if(x_now > x_old){
               if(x_now < grid.getXMax()) {
                   if(y_now > y_old && y_now < grid.getYMax()) new_cell+=grid.getXMax();
                   else if(y_now < y_old && y_now >= 0) new_cell-=grid.getXMax();
                   if(z_now > z_old && z_now < grid.getZMax()) new_cell += grid.getXMax()*grid.getYMax();
                   else if(z_now < z_old && z_now  >= 0) new_cell -= grid.getXMax()*grid.getYMax();
